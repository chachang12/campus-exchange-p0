...
import { HomeIcon, ExploreIcon, ProfileIcon, CourtIcon } from '../components/icons';
  ...
  return (
    ...
    <div className='fixed inset-x-0 bottom-6 flex justify-center'>
      <div className='flex flex-row justify-center space-x-8 backdrop-blur-md bg-opacity-30 bg-dark-blue w-[75%] rounded-full py-2 border border-white border-opacity-40'>
        ...
        <div className={location.pathname === '/messages' ? 'p-1 rounded-full bg-white' : 'p-1 rounded-full border-transparent'}>
          <CourtIcon color={location.pathname === '/messages' ? '#011C40' : 'white'} className='w-[35px] relative z-10' onClick={() => navigate('/messages')} />
        </div>
