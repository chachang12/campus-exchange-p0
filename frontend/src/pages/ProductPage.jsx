      ...
      <div className='flex flex-col md:flex-row md:space-x-8'>
        ...
        <div className='flex flex-col'>
          ...
          {user?._id === creator?._id && (
            <button onClick={() => setIsSettingsOpen(!isSettingsOpen)} className='mt-4 py-2 px-4 outline outline-1 outline-gray-500 rounded-full bg-[#1F1F1F]'>
              Settings
            </button>
          )}
          {isSettingsOpen && (
            <div className='mt-4 p-4 bg-[#1F1F1F] rounded-lg justify-start items-start text-left'>
              <button onClick={() => navigate(`/edit-product/${product._id}`)} className='w-full py-2 px-4 mb-2  text-white rounded text-left'>
                Edit
              </button>
              <button onClick={handleMarkAsSold} className='w-full py-2 px-4 mb-2  text-white rounded text-left'>
                Mark as Sold
              </button>
              <button onClick={handleDeleteProduct} className='w-full py-2 px-4  text-white rounded text-left'>
                Delete
              </button>
            </div>
          )}
